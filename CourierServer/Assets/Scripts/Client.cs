...
public class Client
{
    ...
    public void SendIntoGame(string _playerName, string _team)
    {
        player = NetworkManager.instance.InstantiatePlayer();
        player.Initialize(id, _playerName, _team);

        // Send all players to the new player
        foreach (Client _client in Server.clients.Values)
            if (_client.player != null)
                if (_client.id != id)
                    ServerSend.SpawnPlayer(id, _client.player, _team);

        // Send the new player to all players (including himself)
        foreach (Client _client in Server.clients.Values)
            if (_client.player != null)
                ServerSend.SpawnPlayer(_client.id, player, _team);

        foreach (Gatherer bot in Server.bots.Values)
            if (bot != null)
                ServerSend.SpawnBot(player.id, bot, bot.team);

        Server.player_team[player.id] = player.team;
        player.SetColors(Server.player_team);
    }
    ...
    private void Disconnect()
    {
        ...
        ThreadManager.ExecuteOnMainThread(() =>
        {
            ...
            // if player is holding an item, remove object from that player and allow it to other players to carry it
            if (player.carriedObject != null)
            {
                player.carriedObject.GetComponent<Rigidbody>().useGravity = true;
                player.carriedObject.GetComponent<BoxCollider>().enabled = true;
        ...
        // send everyone that the player disconnected
        ServerSend.PlayerDisconnected(id);
    }

    private void FreeSpawnPosition()
    {
        if (player.team == "blue")
        {
            ...
            foreach (var key in keys)
                if (Server.spawn_points_blue[key] == player.id)
                    Server.spawn_points_blue[key] = 0;
        }
        else if (player.team == "red")
        {
            ...
            var keys = temp_dict.Keys;
            foreach (var key in keys)
                if (Server.spawn_points_red[key] == player.id)
                    Server.spawn_points_red[key] = 0;
        }
    }
}
